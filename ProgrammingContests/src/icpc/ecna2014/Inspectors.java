...
public class Inspectors {
     ...
     * Perform the hungarian algorithm on a matrix, to determine the best way to choose n elements,
     * one from each column and row, such that the maximum sum of elements is achieved.
     * Or: Interpret the matrix as a complete bipartite map from the first index to the second with weights
     * given by the array.
    ...
    public static void main(String[] args) throws IOException {
        ...
        for (int i = 0; i < t; i++) {
            ...
            distMatrix[n - 1][n - 1] = Integer.MIN_VALUE / 2;
            for (int j = 0; j < n - 1; j++) {
                int[] nextDistances = Arrays.stream(br.readLine().split(" "))
                        .mapToInt(x -> Integer.parseInt(x)).toArray();
                distMatrix[j][j] = -10000;
                for (int k = j + 1; k < n; k++) {
                    distMatrix[j][k] = -nextDistances[k - j - 1];
                    distMatrix[k][j] = -nextDistances[k - j - 1];
                }
            }
            hungarianAlgorithm(distMatrix);
            int sum = 0;
            for (int k = 0; k < n; k++) {
                sum -= distMatrix[k][xMatching[k]];
            }
