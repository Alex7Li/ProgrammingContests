 ...
 * METHOD LIST:
 * long bitReverse(int tough.interesting.getN, int k)
...
public class NumberTheoreticTransform {
    ...
    //highest power of 2 that is a factor of medium.original.tough.interesting.getMOD-1
    ...
    private static long[] calcRoots(int k, boolean inverse) {
        long root = NumberTheory.modPow(ROOT, NumberTheory.modPow(2, twoPow - k, MOD),MOD);
        if (inverse) {
            root = NumberTheory.modInv(root, MOD);
        }
    ...
    /**
     * Change the indicies of an array, putting them into bit-reverse Medium.Rudimentary.order.
